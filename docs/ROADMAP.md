...
- ✅ **JSON-RPC sidecar** ([`vibechek/rpc.py`](../vibechek/rpc.py)): 51 methods (28 at first ship, grown since), threadpool dispatch (8 workers), stdout lock, progress + per-track notifications, structured error codes.
...
- ✅ **Correctness pass — fixed every significant finding** (atomic writes, path-traversal guard, organize overwrite-prevention, cancellation coverage, RPC sync guardrail, …). See CHANGELOG beta.4.
- ✅ **Correctness pass — the rest of it** — beta.8 cleared the remaining findings: lossy tag backup now format-complete incl. AIFF/WAV cue frames, dead Direction classifier, opaque UNC/network-share error, racing concurrent index writes, plus a broader sweep (RPC input validation/clamping, `sanitize_folder_name` reserved-name rejection, multi-GPU device-0 pin, atomic WSL shim rewrites, …). See CHANGELOG beta.8.
...
- ✅ **Genre: three classifiers + smart tag reconciliation** — [`vibechek/genres.py`](../vibechek/genres.py) `reconcile_genre` (tier order tag › grounded-web › audio, via `genre_source_policy`); the opt-in **CLAP** pure-audio classifier ([`vibechek/clap_genre.py`](../vibechek/clap_genre.py): a CLAP embedding → distance-weighted kNN over a bundled 2 MB reference, ~2× the Discogs head — ~54% exact / ~69% family on a 74-track web-verified gold corpus); and the opt-in **online genre lookup** ([`vibechek/genre_web.py`](../vibechek/genre_web.py): keyless `ddgs` search → polite fetch of the result pages → bounded regex over the catalog page's structured genre field → artist+title identity gate + chart-bucket refusal + verbatim-quote check, **no model in the loop** — 73.3% exact / 87.2% family on the 86-track adjudicated corpus vs 52.3/70.9 for shipping `prefer_tag`; the earlier local-LLM version of this tier measured 59.3/72.7 and was removed). One-click `setup_clap_engine` / `setup_genre_resolver`; extras `[clap]` / `[resolver]`; default stays Discogs + `prefer_tag` (zero regression). Verified end-to-end in a real WSL analyze + a live WebDriver GUI crawl.
...
- ✅ **Trust UX #2 — inline genre correction (batch approve/revert)** (`resolve_genre_conflicts` RPC) — the review queue is now actionable. Select flagged tracks (or select-all within the "N to review" filter) and **Approve** to accept Vibechek's reconciled genre, or **Revert to tag** to keep the genre already in the file. Either clears the conflict (the track leaves the queue; the queue shows an "all caught up" state when drained) and persists the decision to the saved analysis so it survives a reload — an approved track records `ml_genre_source="approved"` (a human vouched for it). Augment-not-overwrite holds: it **never writes file tags** (that stays the backup-first **Apply ML tags** flow); it only resolves the in-library review state. #3 (tag import as priors) shipped next (below).
- ✅ **Trust UX #3 — tag import as priors** (`import_tag_priors` RPC + [`vibechek/tag_priors.py`](../vibechek/tag_priors.py)) — import a Rekordbox collection XML so the genre you curated there (which never reaches the files) joins reconciliation at the trusted "your tag" tier (`existing_tags.genre_origin="rekordbox"`), written to a `<analysis>.priors.json` sidecar and re-merged on every future analyze; never writes file tags. Embedded key tags + Mixed In Key "Energy N" surface as **read-only context, not overrides** — measured on real libraries, tag keys are less accurate than Vibechek's audio read (49% vs 63% exact), so a disagreeing tag key flags the track instead of silently replacing the better value. Shipped commit `632f06f`; direct Beatport / MixedInKey / Lexicon-export import remain open follow-ups (below).
...
**Shipped in 0.8.0-beta — a silent-degradation sweep that found and fixed 41 defects:**
...
- **Trust-UX line (post-accuracy direction).** Analysis accuracy is at the published ceiling for electronic music (see [docs/COMPETITORS.md](COMPETITORS.md)), so the higher-leverage work for adoption by skeptical pros is *trust*, not more accuracy %. Three staged efforts: **#1 conflict/confidence surfacing — shipped** (see Phase 4); **#2 inline correct/override — shipped** (batch approve/revert from the review queue via `resolve_genre_conflicts`, persisted and never lost; see Phase 4); **#3 tag import as priors — shipped** (Rekordbox collection-XML import via `import_tag_priors` + `vibechek/tag_priors.py`; see Phase 4). Remaining follow-ups below.
- **Beatport / MixedInKey / Lexicon tag import (trust-UX #3 follow-ups, augment-not-overwrite).** "I already have data from $TOOL, use that as ground truth instead of/alongside ML." Rekordbox collection-XML import (genre priors + read-only key / MIK-energy surfacing) **shipped** — see Phase 4. Still open: reading Beatport genre tags and direct MixedInKey / Lexicon exports as further strong priors into `genres.reconcile_genre`.
- **Native-Windows (WSL-free) analyze path — SHIPPED as the Windows default in 0.6.3-beta.** Windows routed ML through WSL only because essentia has no Windows wheel. Now there's `inference_engine="native"`: ONNX inference + a pure-NumPy mel frontend (`vibechek/numpy_frontend.py`, reproduces essentia's `TensorflowInputMusiCNN` bit-close — log-mel L1 0.0000, embedding cosine 1.00000) + a **DSP-only native essentia wheel** (decode/BPM/key) built on Windows from the wo80 MSVC fork (reproducible via `scripts/build_native_essentia_wheel.ps1`, delvewheel-packaged self-contained). Validated end-to-end: a real analyze ran fully in-process, no WSL, no TensorFlow; the wheel's `RhythmExtractor2013`/`KeyExtractor` match WSL-essentia exactly (KEY 12/12, BPM 11/12 on a check). **All three default-flip preconditions shipped** — the cp312 wheel is bundled into the Windows installer (0.6.1-beta), the gold-corpus CI gate landed (`vibechek/gold_gate.py`, commit `dd903ab`), and the default flipped to `native` on Windows in 0.6.3-beta (`config._DEFAULT_INFERENCE_ENGINE`). Open follow-ups: GPU on the native engine (the bundle ships CPU-only ONNX Runtime — switch to `onnx` for GPU) and flipping the default off `essentia_tf` on macOS/Linux (already native-capable via the `onnx` engine, since essentia ships wheels there). See `scripts/native_frontend_parity.py`, `tests/test_native_engine.py`, CHANGELOG.
...
- **CLAP on the native engine — export the audio encoder to ONNX.** CLAP is the one analysis feature the Windows-default native engine can't run, and the reason is torch, not the wheel: `laion_clap` sits on PyTorch (1 GB+ installed — never bundleable into the onefile), so on Windows the CLAP setup installs into a WSL venv the in-process frozen sidecar can physically never import. 0.8.0 made that honest (setup button gated off native with a hint, RPC refuses with the real reason, `ml_genre_classifier` provenance + a fallback warning when a stale config still says clap) — this item makes it *work*. The scoped path is the same migration the EffNet models already survived: export CLAP's **HTSAT audio encoder** to ONNX and run it on the already-bundled ONNX Runtime. Everything else is already torch-free — the kNN reference is pure numpy and ships bundled (`vibechek/clap_assets/`), and the 3×20 s segment pipeline (`clap_genre._segments`) is plain numpy. Deliverables: (1) a conversion script (precedent: `scripts/convert_heads_to_onnx.py`) producing an encoder `.onnx` hosted on the models-onnx mirror with a SHA256 pin (replacing the 2.2 GB torch checkpoint download with a much smaller encoder-only artifact); (2) a parity harness vs the torch reference (precedent: `scripts/onnx_parity.py` / `native_frontend_parity.py`) gating on embedding cosine + genre top-1 over the gold corpus **through the production `knn_predict` path** — the measure-first rule; (3) route `_maybe_load_clap` through the ONNX encoder when available. Payoffs: CLAP's ~2× genre lift reaches the zero-setup Windows default; torch (the heaviest opt-in dependency and the largest RAM budget, 4.5 GB/worker) drops out on every platform — the worker-budget math likely improves too; and it pairs naturally with the DirectML item below, since one runtime then accelerates all inference. Risk: HTSAT's attention/patch ops are the classic tricky-export ops — the parity gate is the whole ballgame, exactly like the EffNet migration.
- **Native-engine GPU via onnxruntime-directml (Windows).** The native bundle deliberately ships CPU-only `onnxruntime` (the CUDA stack is ~2 GB, NVIDIA-only — wrong trade for a onefile installer). `onnxruntime-directml` is the Windows-native cross-vendor answer (DX12: NVIDIA/AMD/Intel, no CUDA toolkit): moderate size cost, and `build_providers` already lists the DirectML EP — the work is bundling the package, hardware-validating the EP (it's wired-but-unverified today), and keeping the worker-budget honesty (registration-gated GPU workers) intact. Pairs with the CLAP-to-ONNX item above: together they make the zero-setup Windows install fully featured AND accelerated through a single runtime.
