...
# drives the existing "Set up WSL" setup UI instead.
...
"""
    ...
    for line in stdout.splitlines():
        ...
        if line.startswith("vibechek_version=") and len(line) > len("vibechek_version="):
            ...
            # visible in the sidecar log; ensure_engine_runtime repairs it
            # automatically on the next analyze.
            log.warning(
...
def upgrade_vibechek_in_wsl(
    ...
    # `import onnxruntime` / `import essentia` still crashes is a status-lie.
    # Verify the venv can import its ML stack; if not, DON'T
...
# ---------------------------------------------------------------------------
#
# DETECT -> SELF-HEAL -> RUN, by design. The user
...
def _autoheal_disabled() -> bool:
    ...
    (unset -> healing enabled): no manual step may be the ONLY
