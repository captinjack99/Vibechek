...
# file and raises with a `vibechek verify-models` fix-it hint. The .json
...
def verify_model_sha256(path: Path, expected: str | None) -> None:
    ...
    if got.lower() != expected.lower():
        ...
        # fix-it instructions are DROPPED: GUI callers re-download
...
def _download_failure_hint(errors: list[str], model_dir: Path) -> str:
    """Pick the RIGHT fix-it hint for a batch of model-download failures.
...
def _needs_download(path: Path, url: str, expected_sha256: str | None = None) -> bool:
    ...
    *On HEAD failure (network error, DNS, server down) the completeness of the
    ...
    against. We no longer treat that "unknown" as "valid"
