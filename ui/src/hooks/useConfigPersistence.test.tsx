/**
 * get_config attaches `config_warnings` when the loader snapped an
 ...
 * choice.
 *
 * A change made inside the 500ms autosave debounce must not be lost on
...
/**
 * Regression test: a failed/fallback get_config overwrote the user's
 * on-disk settings with defaults 500ms later.
 *
...
describe("useConfigPersistence — a load we can't trust must not write defaults back", () => {
  ...
  // The refs above only gate THIS hook's autosave. Other views must not
