...
// ---------------------------------------------------------------------------
// F001 / F020 — the bulk-tag confirm modal must model the tagger's real
...
describe("<LibraryBrowser /> — bulk-tag confirm preview", () => {
  ...
  // F020 — the headline counts FILES, and every genre-written file is already
...
// ---------------------------------------------------------------------------
// F023 — the new-tracks banner must not survive the analyze it launched.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// F024 — "All caught up" is a claim about the LIBRARY, so it may only be made
...
describe("<LibraryBrowser /> — find compatible", () => {
  ...
  // F073 (residual) — energy is a separate head and can fail on its own, so a
...
// ---------------------------------------------------------------------------
// F004 — a failed analyze must hand fail() a closure that re-runs the analyze
...
// ---------------------------------------------------------------------------
// resolve_genre_conflicts wire change (F094): ok:false + reason when nothing
...
// ---------------------------------------------------------------------------
// F057 cross-guards: resolve_genre_conflicts and import_tag_priors both rewrite
