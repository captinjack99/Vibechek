...
export function Settings() {
  ...
  const handleVerifyModels = async () => {
    ...
    try {
      ...
      if (bad.length > 0) {
        // The "Download models now" button lives ~800 lines
  ...
  // slider is based on the model actually in use — when CLAP
  ...
  // the TF CUDA-wheels install block — keeps rendering the PREVIOUS engine's truth until
    ...
    <div className="h-full overflow-auto px-6 py-5">
      ...
      >
        {(() => {
          ...
          // worker allows on the measured pool, so the slider slides to the
          // max workers actually supported.
          const sliderMax =
