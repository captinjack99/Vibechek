...
- **Dependency refresh — the entire dependabot queue (11 PRs) handled.**
  ...
  call site checked by hand. One real migration regression turned up during
  that review and was fixed: Tailwind 4's layer semantics stacked a second
...
- **"Download models" now downloads the right set for the active engine.**
  ...
  the button meant to fix a missing-models error couldn't.
...
- **Hardened the native wheel bundling against silent breakage.** Pin build- and
  ...
  log so they can be checked. (Default engine stays `essentia_tf`; flipping the default to
...
- A fresh end-to-end pass turned up and fixed 19 issues: CLAP-aware worker memory budgeting
...
one-click ONNX engine setup, the full-stack fixes from the beta.10 cycle,
**and a second, deeper pass — driving every RPC method, every CLI command, and
the live GUI across 16 domains — that found and fixed 46 more issues** — see
"Fixed (full-stack sweep)" below.
...
Found by driving every RPC method, every CLI command, and the live GUI
(tauri-driver + WebDriver) against a real library.
...
An end-to-end correctness pass, two new flagship features (FLAC→CDJ
...
Undo journal + the remaining minor and informational fixes.
...
End-to-end correctness pass. Fixed every significant finding.
...
[Unreleased]: https://github.com/captinjack99/Vibechek/compare/v0.9.2-beta...HEAD
[0.9.2-beta]: https://github.com/captinjack99/Vibechek/compare/v0.9.1-beta...v0.9.2-beta
