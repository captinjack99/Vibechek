"""Tests for the shared worker-budget model and hardware-aware resource sizing.
...
def test_clap_wsl_16req_caps_to_2_with_reason() -> None:
    ...
    # The GB math / model name are DEMOTED to cap_detail; the calm
    # cap_reason on the progress line stays free of them.
...
def test_clap_8gb_refuses_with_plain_headline_and_demoted_detail() -> None:
    """8 GB WSL VM, CLAP → nothing fits: refuse. `refusal_reason` is a
...
def test_gpu_not_registrable_yields_zero_gpu_workers_with_reason() -> None:
    """nvidia-smi VRAM used to size "3 GPU workers" while TF couldn't
    register the GPU → all ran on CPU. Now registration gates it:
...
def test_near_empty_vram_floors_to_zero_not_a_doomed_pool() -> None:
    """200 MB free VRAM used to floor to 1 GPU worker that
...
# ---------------------------------------------------------------------------
# Size off what's FREE, not just what the machine HAS
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# A core-count cap must not be reported as a memory cap
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# The per-worker budget has to know how long the tracks are
# ---------------------------------------------------------------------------
...
def test_a_set_heavy_library_shrinks_the_pool_and_says_why() -> None:
    """The scenario this guards against: six workers on 90-minute recorded sets. The
    ...
    # ...and it stays out of the calm headline.
...
# ---------------------------------------------------------------------------
# The slider and the run must apply the SAME availability cap
# ---------------------------------------------------------------------------
