"""Tests for the VRAM-aware GPU worker cap.
...
- `_probe_free_vram_mb` shells out to `nvidia-smi` and returns free VRAM in MB
  ...
  failure. (Summing over-provisioned workers that all landed on GPU 0 → OOM.)
...
def test_probe_returns_device0_on_multi_gpu_by_default() -> None:
    ...
    Summing free VRAM across cards (8000+4000=12000) over-provisioned workers
    that then all piled onto GPU 0 and OOM'd. We now report only the device
...
def test_effnet_crash_degrades_the_track_instead_of_failing_it(monkeypatch, tmp_path) -> None:
    ...
    # throw away tempo and key the track could have had.
...
def test_decoded_buffers_are_released_as_soon_as_they_are_dead(
    ...
    worker `resources.per_worker_mb` sizes at a flat 800 MB.
    """
    import sys
    import types
    import weakref
...
# ---------------------------------------------------------------------------
# The run's own budget is sized against the library's longest track
# ---------------------------------------------------------------------------
