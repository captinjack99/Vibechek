...
def test_preflight_native_engine_served_by_dsp_only_wheel(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # always ships both, so simulate that here (see the onnxruntime
    # importability tests below).
...
# ---------------------------------------------------------------------------
# onnxruntime is what the onnx/native engines actually infer through. It's
# an optional extra, and the import only happens at model-load time — so
# preflight used to say READY for an engine that can't run a track.
# ---------------------------------------------------------------------------
